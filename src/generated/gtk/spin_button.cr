...
module Gtk
  class SpinButton < Entry
    ...
    def adjustment=(__value)
      LibGtk.spin_button_set_adjustment((to_unsafe as LibGtk::SpinButton*), __value)
    end

    def climb_rate=(__value)
      LibGtk.spin_button_set_climb_rate((to_unsafe as LibGtk::SpinButton*), Float64.cast(__value))
    end

    def digits=(__value)
      LibGtk.spin_button_set_digits((to_unsafe as LibGtk::SpinButton*), UInt32.cast(__value))
    end

    def numeric=(__value)
      LibGtk.spin_button_set_numeric((to_unsafe as LibGtk::SpinButton*), Bool.cast(__value))
    end

    def snap_to_ticks=(__value)
      LibGtk.spin_button_set_snap_to_ticks((to_unsafe as LibGtk::SpinButton*), Bool.cast(__value))
    end

    def update_policy=(__value)
      LibGtk.spin_button_set_update_policy((to_unsafe as LibGtk::SpinButton*), __value)
    end

    def value=(__value)
      LibGtk.spin_button_set_value((to_unsafe as LibGtk::SpinButton*), Float64.cast(__value))
    end

    def wrap=(__value)
      LibGtk.spin_button_set_wrap((to_unsafe as LibGtk::SpinButton*), Bool.cast(__value))
    end

    def self.new_internal(adjustment, climb_rate, digits)
      __return_value = LibGtk.spin_button_new((adjustment.to_unsafe as LibGtk::Adjustment*), Float64.cast(climb_rate), UInt32.cast(digits))
    ...
    def configure(adjustment, climb_rate, digits)
      __return_value = LibGtk.spin_button_configure((to_unsafe as LibGtk::SpinButton*), (adjustment.to_unsafe as LibGtk::Adjustment*), Float64.cast(climb_rate), UInt32.cast(digits))
      __return_value
    end
