module Gtk
  class TextAppearance
    ...
    def bg_color
      Gdk::Color.new((to_unsafe.value.bg_color))
    end

    def bg_color=(value : Gdk::Color)
      to_unsafe.value.bg_color = value
    end

    def fg_color
      Gdk::Color.new((to_unsafe.value.fg_color))
    end

    def fg_color=(value : Gdk::Color)
      to_unsafe.value.fg_color = value
    end

    def rise
      (to_unsafe.value.rise)
    end

    def rise=(value : Int32)
      to_unsafe.value.rise = Int32.new(value)
    end

    def underline
      (to_unsafe.value.underline)
    end

    def underline=(value : UInt32)
      to_unsafe.value.underline = UInt32.new(value)
    end

    def strikethrough
      (to_unsafe.value.strikethrough)
    end

    def strikethrough=(value : UInt32)
      to_unsafe.value.strikethrough = UInt32.new(value)
    end

    def draw_bg
      (to_unsafe.value.draw_bg)
    end

    def draw_bg=(value : UInt32)
      to_unsafe.value.draw_bg = UInt32.new(value)
    end

    def inside_selection
      (to_unsafe.value.inside_selection)
    end

    def inside_selection=(value : UInt32)
      to_unsafe.value.inside_selection = UInt32.new(value)
    end

    def is_text
      (to_unsafe.value.is_text)
    end

    def is_text=(value : UInt32)
      to_unsafe.value.is_text = UInt32.new(value)
    end
