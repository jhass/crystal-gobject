module Gtk
  class Gradient
    ...
    def self.new_linear(x0, y0, x1, y1)
      __return_value = LibGtk.gradient_new_linear(Float64.cast(x0), Float64.cast(y0), Float64.cast(x1), Float64.cast(y1))
      Gtk::Gradient.new(__return_value)
    end

    def self.new_radial(x0, y0, radius0, x1, y1, radius1)
      __return_value = LibGtk.gradient_new_radial(Float64.cast(x0), Float64.cast(y0), Float64.cast(radius0), Float64.cast(x1), Float64.cast(y1), Float64.cast(radius1))
      Gtk::Gradient.new(__return_value)
    end

    def add_color_stop(offset, color)
      __return_value = LibGtk.gradient_add_color_stop((to_unsafe as LibGtk::Gradient*), Float64.cast(offset), (color.to_unsafe as LibGtk::SymbolicColor*))
      __return_value
    end
