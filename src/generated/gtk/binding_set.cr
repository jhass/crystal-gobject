module Gtk
  class BindingSet
    ...
    @pointer : Void*
    def initialize(pointer : LibGtk::BindingSet*)
      @pointer = pointer.as(Void*)
    end

    def to_unsafe
      @pointer.not_nil!.as(LibGtk::BindingSet*)
    end

    def activate(keyval, modifiers : Gdk::ModifierType, object)
      __return_value = LibGtk.binding_set_activate(@pointer.as(LibGtk::BindingSet*), UInt32.new(keyval), modifiers, object.to_unsafe.as(LibGObject::Object*))
      __return_value
    end

    def add_path(path_type : Gtk::PathType, path_pattern, priority : Gtk::PathPriorityType)
      LibGtk.binding_set_add_path(@pointer.as(LibGtk::BindingSet*), path_type, path_pattern.to_unsafe, priority)
      nil
    end
