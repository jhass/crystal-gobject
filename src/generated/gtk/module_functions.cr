module Gtk
  def self.accel_groups_activate(object, accel_key, accel_mods)
    __return_value = LibGtk.accel_groups_activate((object.to_unsafe as LibGObject::Object*), UInt32.cast(accel_key), accel_mods)
    __return_value
  end
  ...
  def self.accelerator_get_label(accelerator_key, accelerator_mods)
    __return_value = LibGtk.accelerator_get_label(UInt32.cast(accelerator_key), accelerator_mods)
    raise "Expected string but got null" unless __return_value; String.new(__return_value)
  end

  def self.accelerator_get_label_with_keycode(display, accelerator_key, keycode, accelerator_mods)
    __return_value = LibGtk.accelerator_get_label_with_keycode((display.to_unsafe as LibGdk::Display*), UInt32.cast(accelerator_key), UInt32.cast(keycode), accelerator_mods)
    raise "Expected string but got null" unless __return_value; String.new(__return_value)
  end

  def self.accelerator_name(accelerator_key, accelerator_mods)
    __return_value = LibGtk.accelerator_name(UInt32.cast(accelerator_key), accelerator_mods)
    raise "Expected string but got null" unless __return_value; String.new(__return_value)
  end

  def self.accelerator_name_with_keycode(display, accelerator_key, keycode, accelerator_mods)
    __return_value = LibGtk.accelerator_name_with_keycode((display.to_unsafe as LibGdk::Display*), UInt32.cast(accelerator_key), UInt32.cast(keycode), accelerator_mods)
    raise "Expected string but got null" unless __return_value; String.new(__return_value)
  end

  def self.accelerator_parse(accelerator, accelerator_key, accelerator_mods)
    __return_value = LibGtk.accelerator_parse(accelerator, UInt32.cast(accelerator_key), accelerator_mods)
    __return_value
  end

  def self.accelerator_parse_with_keycode(accelerator, accelerator_key, accelerator_codes, accelerator_mods)
    __return_value = LibGtk.accelerator_parse_with_keycode(accelerator, UInt32.cast(accelerator_key), accelerator_codes, accelerator_mods)
    __return_value
  end
  ...
  def self.accelerator_valid(keyval, modifiers)
    __return_value = LibGtk.accelerator_valid(UInt32.cast(keyval), modifiers)
    __return_value
  end

  def self.alternative_dialog_button_order(screen)
    __return_value = LibGtk.alternative_dialog_button_order((screen.to_unsafe as LibGdk::Screen*))
    __return_value
  end
  ...
  def self.bindings_activate(object, keyval, modifiers)
    __return_value = LibGtk.bindings_activate((object.to_unsafe as LibGObject::Object*), UInt32.cast(keyval), modifiers)
    __return_value
  end
  ...
  def self.check_version(required_major, required_minor, required_micro)
    __return_value = LibGtk.check_version(UInt32.cast(required_major), UInt32.cast(required_minor), UInt32.cast(required_micro))
    raise "Expected string but got null" unless __return_value; String.new(__return_value)
  end
  ...
  def self.device_grab_add(widget, device, block_others)
    __return_value = LibGtk.device_grab_add((widget.to_unsafe as LibGtk::Widget*), (device.to_unsafe as LibGdk::Device*), Bool.cast(block_others))
    __return_value
  end
  ...
  def self.distribute_natural_allocation(extra_space, n_requested_sizes, sizes)
    __return_value = LibGtk.distribute_natural_allocation(Int32.cast(extra_space), UInt32.cast(n_requested_sizes), (sizes.to_unsafe as LibGtk::RequestedSize*))
    __return_value
  end
  ...
  def self.drag_set_icon_gicon(context, icon, hot_x, hot_y)
    __return_value = LibGtk.drag_set_icon_gicon((context.to_unsafe as LibGdk::DragContext*), (icon.to_unsafe as LibGio::Icon*), Int32.cast(hot_x), Int32.cast(hot_y))
    __return_value
  end

  def self.drag_set_icon_name(context, icon_name, hot_x, hot_y)
    __return_value = LibGtk.drag_set_icon_name((context.to_unsafe as LibGdk::DragContext*), icon_name, Int32.cast(hot_x), Int32.cast(hot_y))
    __return_value
  end

  def self.drag_set_icon_pixbuf(context, pixbuf, hot_x, hot_y)
    __return_value = LibGtk.drag_set_icon_pixbuf((context.to_unsafe as LibGdk::DragContext*), (pixbuf.to_unsafe as LibGdkPixbuf::Pixbuf*), Int32.cast(hot_x), Int32.cast(hot_y))
    __return_value
  end

  def self.drag_set_icon_stock(context, stock_id, hot_x, hot_y)
    __return_value = LibGtk.drag_set_icon_stock((context.to_unsafe as LibGdk::DragContext*), stock_id, Int32.cast(hot_x), Int32.cast(hot_y))
    __return_value
  end
  ...
  def self.drag_set_icon_widget(context, widget, hot_x, hot_y)
    __return_value = LibGtk.drag_set_icon_widget((context.to_unsafe as LibGdk::DragContext*), (widget.to_unsafe as LibGtk::Widget*), Int32.cast(hot_x), Int32.cast(hot_y))
    __return_value
  end

  def self.draw_insertion_cursor(widget, cr, location, is_primary, direction, draw_arrow)
    __return_value = LibGtk.draw_insertion_cursor((widget.to_unsafe as LibGtk::Widget*), (cr.to_unsafe as LibCairo::Context*), (location.to_unsafe as LibCairo::RectangleInt*), Bool.cast(is_primary), direction, Bool.cast(draw_arrow))
    __return_value
  end
  ...
  def self.icon_size_get_name(size)
    __return_value = LibGtk.icon_size_get_name(Int32.cast(size))
    raise "Expected string but got null" unless __return_value; String.new(__return_value)
  end

  def self.icon_size_lookup(size, width, height)
    __return_value = LibGtk.icon_size_lookup(Int32.cast(size), Int32.cast(width), Int32.cast(height))
    __return_value
  end

  def self.icon_size_lookup_for_settings(settings, size, width, height)
    __return_value = LibGtk.icon_size_lookup_for_settings((settings.to_unsafe as LibGtk::Settings*), Int32.cast(size), Int32.cast(width), Int32.cast(height))
    __return_value
  end

  def self.icon_size_register(name, width, height)
    __return_value = LibGtk.icon_size_register(name, Int32.cast(width), Int32.cast(height))
    __return_value
  end

  def self.icon_size_register_alias(_alias, target)
    __return_value = LibGtk.icon_size_register_alias(_alias, Int32.cast(target))
    __return_value
  end
  ...
  def self.init(argc, argv)
    __return_value = LibGtk.init(Int32.cast(argc), argv)
    __return_value
  end

  def self.init_check(argc, argv)
    __return_value = LibGtk.init_check(Int32.cast(argc), argv)
    __return_value
  end

  def self.init_with_args(argc, argv, parameter_string, entries, translation_domain)
    __error = Pointer(LibGLib::Error).null
    __return_value = LibGtk.init_with_args(Int32.cast(argc), argv, parameter_string, entries, translation_domain, pointerof(__error))
    GLib::Error.assert __error
    __return_value
  end

  def self.key_snooper_remove(snooper_handler_id)
    __return_value = LibGtk.key_snooper_remove(UInt32.cast(snooper_handler_id))
    __return_value
  end
  ...
  def self.main_iteration_do(blocking)
    __return_value = LibGtk.main_iteration_do(Bool.cast(blocking))
    __return_value
  end
  ...
  def self.paint_arrow(style, cr, state_type, shadow_type, widget, detail, arrow_type, fill, x, y, width, height)
    __return_value = LibGtk.paint_arrow((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, shadow_type, (widget.to_unsafe as LibGtk::Widget*), detail, arrow_type, Bool.cast(fill), Int32.cast(x), Int32.cast(y), Int32.cast(width), Int32.cast(height))
    __return_value
  end

  def self.paint_box(style, cr, state_type, shadow_type, widget, detail, x, y, width, height)
    __return_value = LibGtk.paint_box((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, shadow_type, (widget.to_unsafe as LibGtk::Widget*), detail, Int32.cast(x), Int32.cast(y), Int32.cast(width), Int32.cast(height))
    __return_value
  end

  def self.paint_box_gap(style, cr, state_type, shadow_type, widget, detail, x, y, width, height, gap_side, gap_x, gap_width)
    __return_value = LibGtk.paint_box_gap((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, shadow_type, (widget.to_unsafe as LibGtk::Widget*), detail, Int32.cast(x), Int32.cast(y), Int32.cast(width), Int32.cast(height), gap_side, Int32.cast(gap_x), Int32.cast(gap_width))
    __return_value
  end

  def self.paint_check(style, cr, state_type, shadow_type, widget, detail, x, y, width, height)
    __return_value = LibGtk.paint_check((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, shadow_type, (widget.to_unsafe as LibGtk::Widget*), detail, Int32.cast(x), Int32.cast(y), Int32.cast(width), Int32.cast(height))
    __return_value
  end

  def self.paint_diamond(style, cr, state_type, shadow_type, widget, detail, x, y, width, height)
    __return_value = LibGtk.paint_diamond((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, shadow_type, (widget.to_unsafe as LibGtk::Widget*), detail, Int32.cast(x), Int32.cast(y), Int32.cast(width), Int32.cast(height))
    __return_value
  end

  def self.paint_expander(style, cr, state_type, widget, detail, x, y, expander_style)
    __return_value = LibGtk.paint_expander((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, (widget.to_unsafe as LibGtk::Widget*), detail, Int32.cast(x), Int32.cast(y), expander_style)
    __return_value
  end

  def self.paint_extension(style, cr, state_type, shadow_type, widget, detail, x, y, width, height, gap_side)
    __return_value = LibGtk.paint_extension((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, shadow_type, (widget.to_unsafe as LibGtk::Widget*), detail, Int32.cast(x), Int32.cast(y), Int32.cast(width), Int32.cast(height), gap_side)
    __return_value
  end

  def self.paint_flat_box(style, cr, state_type, shadow_type, widget, detail, x, y, width, height)
    __return_value = LibGtk.paint_flat_box((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, shadow_type, (widget.to_unsafe as LibGtk::Widget*), detail, Int32.cast(x), Int32.cast(y), Int32.cast(width), Int32.cast(height))
    __return_value
  end

  def self.paint_focus(style, cr, state_type, widget, detail, x, y, width, height)
    __return_value = LibGtk.paint_focus((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, (widget.to_unsafe as LibGtk::Widget*), detail, Int32.cast(x), Int32.cast(y), Int32.cast(width), Int32.cast(height))
    __return_value
  end

  def self.paint_handle(style, cr, state_type, shadow_type, widget, detail, x, y, width, height, orientation)
    __return_value = LibGtk.paint_handle((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, shadow_type, (widget.to_unsafe as LibGtk::Widget*), detail, Int32.cast(x), Int32.cast(y), Int32.cast(width), Int32.cast(height), orientation)
    __return_value
  end

  def self.paint_hline(style, cr, state_type, widget, detail, x1, x2, y)
    __return_value = LibGtk.paint_hline((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, (widget.to_unsafe as LibGtk::Widget*), detail, Int32.cast(x1), Int32.cast(x2), Int32.cast(y))
    __return_value
  end

  def self.paint_layout(style, cr, state_type, use_text, widget, detail, x, y, layout)
    __return_value = LibGtk.paint_layout((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, Bool.cast(use_text), (widget.to_unsafe as LibGtk::Widget*), detail, Int32.cast(x), Int32.cast(y), (layout.to_unsafe as LibPango::Layout*))
    __return_value
  end

  def self.paint_option(style, cr, state_type, shadow_type, widget, detail, x, y, width, height)
    __return_value = LibGtk.paint_option((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, shadow_type, (widget.to_unsafe as LibGtk::Widget*), detail, Int32.cast(x), Int32.cast(y), Int32.cast(width), Int32.cast(height))
    __return_value
  end

  def self.paint_resize_grip(style, cr, state_type, widget, detail, edge, x, y, width, height)
    __return_value = LibGtk.paint_resize_grip((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, (widget.to_unsafe as LibGtk::Widget*), detail, edge, Int32.cast(x), Int32.cast(y), Int32.cast(width), Int32.cast(height))
    __return_value
  end

  def self.paint_shadow(style, cr, state_type, shadow_type, widget, detail, x, y, width, height)
    __return_value = LibGtk.paint_shadow((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, shadow_type, (widget.to_unsafe as LibGtk::Widget*), detail, Int32.cast(x), Int32.cast(y), Int32.cast(width), Int32.cast(height))
    __return_value
  end

  def self.paint_shadow_gap(style, cr, state_type, shadow_type, widget, detail, x, y, width, height, gap_side, gap_x, gap_width)
    __return_value = LibGtk.paint_shadow_gap((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, shadow_type, (widget.to_unsafe as LibGtk::Widget*), detail, Int32.cast(x), Int32.cast(y), Int32.cast(width), Int32.cast(height), gap_side, Int32.cast(gap_x), Int32.cast(gap_width))
    __return_value
  end

  def self.paint_slider(style, cr, state_type, shadow_type, widget, detail, x, y, width, height, orientation)
    __return_value = LibGtk.paint_slider((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, shadow_type, (widget.to_unsafe as LibGtk::Widget*), detail, Int32.cast(x), Int32.cast(y), Int32.cast(width), Int32.cast(height), orientation)
    __return_value
  end

  def self.paint_spinner(style, cr, state_type, widget, detail, step, x, y, width, height)
    __return_value = LibGtk.paint_spinner((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, (widget.to_unsafe as LibGtk::Widget*), detail, UInt32.cast(step), Int32.cast(x), Int32.cast(y), Int32.cast(width), Int32.cast(height))
    __return_value
  end

  def self.paint_tab(style, cr, state_type, shadow_type, widget, detail, x, y, width, height)
    __return_value = LibGtk.paint_tab((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, shadow_type, (widget.to_unsafe as LibGtk::Widget*), detail, Int32.cast(x), Int32.cast(y), Int32.cast(width), Int32.cast(height))
    __return_value
  end

  def self.paint_vline(style, cr, state_type, widget, detail, y1, y2, x)
    __return_value = LibGtk.paint_vline((style.to_unsafe as LibGtk::Style*), (cr.to_unsafe as LibCairo::Context*), state_type, (widget.to_unsafe as LibGtk::Widget*), detail, Int32.cast(y1), Int32.cast(y2), Int32.cast(x))
    __return_value
  end
  ...
  def self.parse_args(argc, argv)
    __return_value = LibGtk.parse_args(Int32.cast(argc), argv)
    __return_value
  end
  ...
  def self.print_run_page_setup_dialog(parent, page_setup, settings)
    __return_value = LibGtk.print_run_page_setup_dialog((parent.to_unsafe as LibGtk::Window*), (page_setup.to_unsafe as LibGtk::PageSetup*), (settings.to_unsafe as LibGtk::PrintSettings*))
    Gtk::PageSetup.new(__return_value)
  end

  def self.print_run_page_setup_dialog_async(parent, page_setup, settings, done_cb, data)
    __return_value = LibGtk.print_run_page_setup_dialog_async((parent.to_unsafe as LibGtk::Window*), (page_setup.to_unsafe as LibGtk::PageSetup*), (settings.to_unsafe as LibGtk::PrintSettings*), done_cb, data)
    __return_value
  end
  ...
  def self.rc_get_style_by_paths(settings, widget_path, class_path, type)
    __return_value = LibGtk.rc_get_style_by_paths((settings.to_unsafe as LibGtk::Settings*), widget_path, class_path, UInt64.cast(type))
  ...
  def self.rc_parse_color_full(scanner, style, color)
    __return_value = LibGtk.rc_parse_color_full((scanner.to_unsafe as LibGLib::Scanner*), (style.to_unsafe as LibGtk::RcStyle*), color)
    __return_value
  end
  ...
  def self.rc_reparse_all_for_settings(settings, force_load)
    __return_value = LibGtk.rc_reparse_all_for_settings((settings.to_unsafe as LibGtk::Settings*), Bool.cast(force_load))
    __return_value
  end
  ...
  def self.render_activity(context, cr, x, y, width, height)
    __return_value = LibGtk.render_activity((context.to_unsafe as LibGtk::StyleContext*), (cr.to_unsafe as LibCairo::Context*), Float64.cast(x), Float64.cast(y), Float64.cast(width), Float64.cast(height))
    __return_value
  end

  def self.render_arrow(context, cr, angle, x, y, size)
    __return_value = LibGtk.render_arrow((context.to_unsafe as LibGtk::StyleContext*), (cr.to_unsafe as LibCairo::Context*), Float64.cast(angle), Float64.cast(x), Float64.cast(y), Float64.cast(size))
    __return_value
  end

  def self.render_background(context, cr, x, y, width, height)
    __return_value = LibGtk.render_background((context.to_unsafe as LibGtk::StyleContext*), (cr.to_unsafe as LibCairo::Context*), Float64.cast(x), Float64.cast(y), Float64.cast(width), Float64.cast(height))
    __return_value
  end

  def self.render_check(context, cr, x, y, width, height)
    __return_value = LibGtk.render_check((context.to_unsafe as LibGtk::StyleContext*), (cr.to_unsafe as LibCairo::Context*), Float64.cast(x), Float64.cast(y), Float64.cast(width), Float64.cast(height))
    __return_value
  end

  def self.render_expander(context, cr, x, y, width, height)
    __return_value = LibGtk.render_expander((context.to_unsafe as LibGtk::StyleContext*), (cr.to_unsafe as LibCairo::Context*), Float64.cast(x), Float64.cast(y), Float64.cast(width), Float64.cast(height))
    __return_value
  end

  def self.render_extension(context, cr, x, y, width, height, gap_side)
    __return_value = LibGtk.render_extension((context.to_unsafe as LibGtk::StyleContext*), (cr.to_unsafe as LibCairo::Context*), Float64.cast(x), Float64.cast(y), Float64.cast(width), Float64.cast(height), gap_side)
    __return_value
  end

  def self.render_focus(context, cr, x, y, width, height)
    __return_value = LibGtk.render_focus((context.to_unsafe as LibGtk::StyleContext*), (cr.to_unsafe as LibCairo::Context*), Float64.cast(x), Float64.cast(y), Float64.cast(width), Float64.cast(height))
    __return_value
  end

  def self.render_frame(context, cr, x, y, width, height)
    __return_value = LibGtk.render_frame((context.to_unsafe as LibGtk::StyleContext*), (cr.to_unsafe as LibCairo::Context*), Float64.cast(x), Float64.cast(y), Float64.cast(width), Float64.cast(height))
    __return_value
  end

  def self.render_frame_gap(context, cr, x, y, width, height, gap_side, xy0_gap, xy1_gap)
    __return_value = LibGtk.render_frame_gap((context.to_unsafe as LibGtk::StyleContext*), (cr.to_unsafe as LibCairo::Context*), Float64.cast(x), Float64.cast(y), Float64.cast(width), Float64.cast(height), gap_side, Float64.cast(xy0_gap), Float64.cast(xy1_gap))
    __return_value
  end

  def self.render_handle(context, cr, x, y, width, height)
    __return_value = LibGtk.render_handle((context.to_unsafe as LibGtk::StyleContext*), (cr.to_unsafe as LibCairo::Context*), Float64.cast(x), Float64.cast(y), Float64.cast(width), Float64.cast(height))
    __return_value
  end

  def self.render_icon(context, cr, pixbuf, x, y)
    __return_value = LibGtk.render_icon((context.to_unsafe as LibGtk::StyleContext*), (cr.to_unsafe as LibCairo::Context*), (pixbuf.to_unsafe as LibGdkPixbuf::Pixbuf*), Float64.cast(x), Float64.cast(y))
    __return_value
  end

  def self.render_icon_pixbuf(context, source, size)
    __return_value = LibGtk.render_icon_pixbuf((context.to_unsafe as LibGtk::StyleContext*), (source.to_unsafe as LibGtk::IconSource*), Int32.cast(size))
    GdkPixbuf::Pixbuf.new(__return_value)
  end

  def self.render_icon_surface(context, cr, surface, x, y)
    __return_value = LibGtk.render_icon_surface((context.to_unsafe as LibGtk::StyleContext*), (cr.to_unsafe as LibCairo::Context*), (surface.to_unsafe as LibCairo::Surface*), Float64.cast(x), Float64.cast(y))
    __return_value
  end

  def self.render_insertion_cursor(context, cr, x, y, layout, index, direction)
    __return_value = LibGtk.render_insertion_cursor((context.to_unsafe as LibGtk::StyleContext*), (cr.to_unsafe as LibCairo::Context*), Float64.cast(x), Float64.cast(y), (layout.to_unsafe as LibPango::Layout*), Int32.cast(index), direction)
    __return_value
  end

  def self.render_layout(context, cr, x, y, layout)
    __return_value = LibGtk.render_layout((context.to_unsafe as LibGtk::StyleContext*), (cr.to_unsafe as LibCairo::Context*), Float64.cast(x), Float64.cast(y), (layout.to_unsafe as LibPango::Layout*))
    __return_value
  end

  def self.render_line(context, cr, x0, y0, x1, y1)
    __return_value = LibGtk.render_line((context.to_unsafe as LibGtk::StyleContext*), (cr.to_unsafe as LibCairo::Context*), Float64.cast(x0), Float64.cast(y0), Float64.cast(x1), Float64.cast(y1))
    __return_value
  end

  def self.render_option(context, cr, x, y, width, height)
    __return_value = LibGtk.render_option((context.to_unsafe as LibGtk::StyleContext*), (cr.to_unsafe as LibCairo::Context*), Float64.cast(x), Float64.cast(y), Float64.cast(width), Float64.cast(height))
    __return_value
  end

  def self.render_slider(context, cr, x, y, width, height, orientation)
    __return_value = LibGtk.render_slider((context.to_unsafe as LibGtk::StyleContext*), (cr.to_unsafe as LibCairo::Context*), Float64.cast(x), Float64.cast(y), Float64.cast(width), Float64.cast(height), orientation)
    __return_value
  end

  def self.rgb_to_hsv(r, g, b, h, s, v)
    __return_value = LibGtk.rgb_to_hsv(Float64.cast(r), Float64.cast(g), Float64.cast(b), Float64.cast(h), Float64.cast(s), Float64.cast(v))
    __return_value
  end

  def self.selection_add_target(widget, selection, target, info)
    __return_value = LibGtk.selection_add_target((widget.to_unsafe as LibGtk::Widget*), (selection.to_unsafe as LibGdk::Atom*), (target.to_unsafe as LibGdk::Atom*), UInt32.cast(info))
    __return_value
  end

  def self.selection_add_targets(widget, selection, targets, ntargets)
    __return_value = LibGtk.selection_add_targets((widget.to_unsafe as LibGtk::Widget*), (selection.to_unsafe as LibGdk::Atom*), targets, UInt32.cast(ntargets))
    __return_value
  end
  ...
  def self.selection_convert(widget, selection, target, time)
    __return_value = LibGtk.selection_convert((widget.to_unsafe as LibGtk::Widget*), (selection.to_unsafe as LibGdk::Atom*), (target.to_unsafe as LibGdk::Atom*), UInt32.cast(time))
    __return_value
  end

  def self.selection_owner_set(widget, selection, time)
    __return_value = LibGtk.selection_owner_set((widget.to_unsafe as LibGtk::Widget*), (selection.to_unsafe as LibGdk::Atom*), UInt32.cast(time))
    __return_value
  end

  def self.selection_owner_set_for_display(display, widget, selection, time)
    __return_value = LibGtk.selection_owner_set_for_display((display.to_unsafe as LibGdk::Display*), (widget.to_unsafe as LibGtk::Widget*), (selection.to_unsafe as LibGdk::Atom*), UInt32.cast(time))
    __return_value
  end
  ...
  def self.debug_flags=(flags)
    __return_value = LibGtk.set_debug_flags(UInt32.cast(flags))
    __return_value
  end

  def self.show_uri(screen, uri, timestamp)
    __error = Pointer(LibGLib::Error).null
    __return_value = LibGtk.show_uri((screen.to_unsafe as LibGdk::Screen*), uri, UInt32.cast(timestamp), pointerof(__error))
    GLib::Error.assert __error
    __return_value
  end

  def self.stock_add(items, n_items)
    __return_value = LibGtk.stock_add(items, UInt32.cast(n_items))
    __return_value
  end

  def self.stock_add_static(items, n_items)
    __return_value = LibGtk.stock_add_static(items, UInt32.cast(n_items))
    __return_value
  end
  ...
  def self.target_table_free(targets, n_targets)
    __return_value = LibGtk.target_table_free(targets, Int32.cast(n_targets))
    __return_value
  end
  ...
  def self.target_table_new_from_list(list, n_targets)
    ...
    PointerIterator.new(__return_value) {|__item_24| Gtk::TargetEntry.new(__item_24) }
  end

  def self.targets_include_image(targets, n_targets, writable)
    __return_value = LibGtk.targets_include_image(targets, Int32.cast(n_targets), Bool.cast(writable))
    __return_value
  end

  def self.targets_include_rich_text(targets, n_targets, buffer)
    __return_value = LibGtk.targets_include_rich_text(targets, Int32.cast(n_targets), (buffer.to_unsafe as LibGtk::TextBuffer*))
    __return_value
  end

  def self.targets_include_text(targets, n_targets)
    __return_value = LibGtk.targets_include_text(targets, Int32.cast(n_targets))
    __return_value
  end

  def self.targets_include_uri(targets, n_targets)
    __return_value = LibGtk.targets_include_uri(targets, Int32.cast(n_targets))
    __return_value
  end
  ...
  def self.test_find_sibling(base_widget, widget_type)
    __return_value = LibGtk.test_find_sibling((base_widget.to_unsafe as LibGtk::Widget*), UInt64.cast(widget_type))
    Gtk::Widget.new(__return_value)
  end

  def self.test_find_widget(widget, label_pattern, widget_type)
    __return_value = LibGtk.test_find_widget((widget.to_unsafe as LibGtk::Widget*), label_pattern, UInt64.cast(widget_type))
    Gtk::Widget.new(__return_value)
  end
  ...
  def self.test_slider_set_perc(widget, percentage)
    __return_value = LibGtk.test_slider_set_perc((widget.to_unsafe as LibGtk::Widget*), Float64.cast(percentage))
    __return_value
  end

  def self.test_spin_button_click(spinner, button, upwards)
    __return_value = LibGtk.test_spin_button_click((spinner.to_unsafe as LibGtk::SpinButton*), UInt32.cast(button), Bool.cast(upwards))
    __return_value
  end
  ...
  def self.test_widget_click(widget, button, modifiers)
    __return_value = LibGtk.test_widget_click((widget.to_unsafe as LibGtk::Widget*), UInt32.cast(button), modifiers)
    __return_value
  end

  def self.test_widget_send_key(widget, keyval, modifiers)
    __return_value = LibGtk.test_widget_send_key((widget.to_unsafe as LibGtk::Widget*), UInt32.cast(keyval), modifiers)
    __return_value
  end
