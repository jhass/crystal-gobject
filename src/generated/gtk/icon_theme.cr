module Gtk
  class IconTheme < GObject::Object
    ...
    def list_icons(context)
      __return_value = LibGtk.icon_theme_list_icons(to_unsafe.as(LibGtk::IconTheme*), context ? context.to_unsafe : nil)
      GLib::ListIterator(String, UInt8**).new(GLib::SList.new(__return_value.as(LibGLib::List*)))
    end
    ...
    def load_surface(icon_name, size, scale, for_window, flags : Gtk::IconLookupFlags)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGtk.icon_theme_load_surface(to_unsafe.as(LibGtk::IconTheme*), icon_name.to_unsafe, Int32.new(size), Int32.new(scale), for_window ? for_window.to_unsafe.as(LibGdk::Window*) : nil, flags, pointerof(__error))
      GLib::Error.assert __error
