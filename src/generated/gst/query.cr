module Gst
  class Query
    ...
    def add_allocation_meta(api, params)
      LibGst.query_add_allocation_meta(to_unsafe.as(LibGst::Query*), UInt64.new(api), params ? params.to_unsafe.as(LibGst::Structure*) : nil)
      nil
    end

    def add_allocation_param(allocator, params)
      LibGst.query_add_allocation_param(to_unsafe.as(LibGst::Query*), allocator ? allocator.to_unsafe.as(LibGst::Allocator*) : nil, params ? params.to_unsafe.as(LibGst::AllocationParams*) : nil)
      nil
    end

    def add_allocation_pool(pool, size, min_buffers, max_buffers)
      LibGst.query_add_allocation_pool(to_unsafe.as(LibGst::Query*), pool ? pool.to_unsafe.as(LibGst::BufferPool*) : nil, UInt32.new(size), UInt32.new(min_buffers), UInt32.new(max_buffers))
      nil
    end
    ...
    def add_scheduling_mode(mode : Gst::PadMode)
      LibGst.query_add_scheduling_mode(to_unsafe.as(LibGst::Query*), mode)
      nil
    end
    ...
    def parse_accept_caps(caps)
      LibGst.query_parse_accept_caps(to_unsafe.as(LibGst::Query*), caps)
      nil
    end

    def parse_accept_caps_result(result)
      LibGst.query_parse_accept_caps_result(to_unsafe.as(LibGst::Query*), result)
      nil
    end

    def parse_allocation(caps, need_pool)
      LibGst.query_parse_allocation(to_unsafe.as(LibGst::Query*), caps, need_pool)
      nil
    end

    def parse_buffering_percent(busy, percent)
      LibGst.query_parse_buffering_percent(to_unsafe.as(LibGst::Query*), busy, percent)
      nil
    end

    def parse_buffering_range(format : Gst::Format?, start, stop, estimated_total)
      LibGst.query_parse_buffering_range(to_unsafe.as(LibGst::Query*), format, start, stop, estimated_total)
      nil
    end

    def parse_buffering_stats(mode : Gst::BufferingMode?, avg_in, avg_out, buffering_left)
      LibGst.query_parse_buffering_stats(to_unsafe.as(LibGst::Query*), mode, avg_in, avg_out, buffering_left)
      nil
    end

    def parse_caps(filter)
      LibGst.query_parse_caps(to_unsafe.as(LibGst::Query*), filter)
      nil
    end

    def parse_caps_result(caps)
      LibGst.query_parse_caps_result(to_unsafe.as(LibGst::Query*), caps)
      nil
    end

    def parse_context(context)
      LibGst.query_parse_context(to_unsafe.as(LibGst::Query*), context)
      nil
    end
    ...
    def parse_convert(src_format : Gst::Format?, src_value, dest_format : Gst::Format?, dest_value)
      LibGst.query_parse_convert(to_unsafe.as(LibGst::Query*), src_format, src_value, dest_format, dest_value)
      nil
    end

    def parse_duration(format : Gst::Format?, duration)
      LibGst.query_parse_duration(to_unsafe.as(LibGst::Query*), format, duration)
      nil
    end

    def parse_latency(live, min_latency, max_latency)
      LibGst.query_parse_latency(to_unsafe.as(LibGst::Query*), live, min_latency, max_latency)
      nil
    end

    def parse_n_formats(n_formats)
      LibGst.query_parse_n_formats(to_unsafe.as(LibGst::Query*), n_formats)
      nil
    end
    ...
    def parse_nth_allocation_param(index, allocator, params)
      LibGst.query_parse_nth_allocation_param(to_unsafe.as(LibGst::Query*), UInt32.new(index), allocator, params)
      nil
    end

    def parse_nth_allocation_pool(index, pool, size, min_buffers, max_buffers)
      LibGst.query_parse_nth_allocation_pool(to_unsafe.as(LibGst::Query*), UInt32.new(index), pool, size, min_buffers, max_buffers)
      nil
    end
    ...
    def parse_nth_format(nth, format : Gst::Format?)
      LibGst.query_parse_nth_format(to_unsafe.as(LibGst::Query*), nth, format)
      nil
    end
    ...
    def parse_position(format : Gst::Format?, cur)
      LibGst.query_parse_position(to_unsafe.as(LibGst::Query*), format, cur)
      nil
    end

    def parse_scheduling(flags : Gst::SchedulingFlags?, minsize, maxsize, align)
      LibGst.query_parse_scheduling(to_unsafe.as(LibGst::Query*), flags, minsize, maxsize, align)
      nil
    end

    def parse_seeking(format : Gst::Format?, seekable, segment_start, segment_end)
      LibGst.query_parse_seeking(to_unsafe.as(LibGst::Query*), format, seekable, segment_start, segment_end)
      nil
    end

    def parse_segment(rate, format : Gst::Format?, start_value, stop_value)
      LibGst.query_parse_segment(to_unsafe.as(LibGst::Query*), rate, format, start_value, stop_value)
      nil
    end

    def parse_uri(uri)
      LibGst.query_parse_uri(to_unsafe.as(LibGst::Query*), uri)
      nil
    end

    def parse_uri_redirection(uri)
      LibGst.query_parse_uri_redirection(to_unsafe.as(LibGst::Query*), uri)
      nil
    end

    def parse_uri_redirection_permanent(permanent)
      LibGst.query_parse_uri_redirection_permanent(to_unsafe.as(LibGst::Query*), permanent)
      nil
    end

    def remove_nth_allocation_meta(index)
      LibGst.query_remove_nth_allocation_meta(to_unsafe.as(LibGst::Query*), UInt32.new(index))
      nil
    end

    def remove_nth_allocation_param(index)
      LibGst.query_remove_nth_allocation_param(to_unsafe.as(LibGst::Query*), UInt32.new(index))
      nil
    end

    def remove_nth_allocation_pool(index)
      LibGst.query_remove_nth_allocation_pool(to_unsafe.as(LibGst::Query*), UInt32.new(index))
      nil
    end

    def accept_caps_result=(result)
      LibGst.query_set_accept_caps_result(to_unsafe.as(LibGst::Query*), result)
      nil
    end

    def set_buffering_percent(busy, percent)
      LibGst.query_set_buffering_percent(to_unsafe.as(LibGst::Query*), busy, Int32.new(percent))
      nil
    end

    def set_buffering_range(format : Gst::Format, start, stop, estimated_total)
      LibGst.query_set_buffering_range(to_unsafe.as(LibGst::Query*), format, Int64.new(start), Int64.new(stop), Int64.new(estimated_total))
      nil
    end

    def set_buffering_stats(mode : Gst::BufferingMode, avg_in, avg_out, buffering_left)
      LibGst.query_set_buffering_stats(to_unsafe.as(LibGst::Query*), mode, Int32.new(avg_in), Int32.new(avg_out), Int64.new(buffering_left))
      nil
    end

    def caps_result=(caps)
      LibGst.query_set_caps_result(to_unsafe.as(LibGst::Query*), caps.to_unsafe.as(LibGst::Caps*))
      nil
    end

    def context=(context)
      LibGst.query_set_context(to_unsafe.as(LibGst::Query*), context.to_unsafe.as(LibGst::Context*))
      nil
    end

    def set_convert(src_format : Gst::Format, src_value, dest_format : Gst::Format, dest_value)
      LibGst.query_set_convert(to_unsafe.as(LibGst::Query*), src_format, Int64.new(src_value), dest_format, Int64.new(dest_value))
      nil
    end

    def set_duration(format : Gst::Format, duration)
      LibGst.query_set_duration(to_unsafe.as(LibGst::Query*), format, Int64.new(duration))
      nil
    end

    def set_formatsv(n_formats, formats)
      LibGst.query_set_formatsv(to_unsafe.as(LibGst::Query*), Int32.new(n_formats), formats)
      nil
    end

    def set_latency(live, min_latency, max_latency)
      LibGst.query_set_latency(to_unsafe.as(LibGst::Query*), live, UInt64.new(min_latency), UInt64.new(max_latency))
      nil
    end

    def set_nth_allocation_param(index, allocator, params)
      LibGst.query_set_nth_allocation_param(to_unsafe.as(LibGst::Query*), UInt32.new(index), allocator ? allocator.to_unsafe.as(LibGst::Allocator*) : nil, params ? params.to_unsafe.as(LibGst::AllocationParams*) : nil)
      nil
    end

    def set_nth_allocation_pool(index, pool, size, min_buffers, max_buffers)
      LibGst.query_set_nth_allocation_pool(to_unsafe.as(LibGst::Query*), UInt32.new(index), pool ? pool.to_unsafe.as(LibGst::BufferPool*) : nil, UInt32.new(size), UInt32.new(min_buffers), UInt32.new(max_buffers))
      nil
    end

    def set_position(format : Gst::Format, cur)
      LibGst.query_set_position(to_unsafe.as(LibGst::Query*), format, Int64.new(cur))
      nil
    end

    def set_scheduling(flags : Gst::SchedulingFlags, minsize, maxsize, align)
      LibGst.query_set_scheduling(to_unsafe.as(LibGst::Query*), flags, Int32.new(minsize), Int32.new(maxsize), Int32.new(align))
      nil
    end

    def set_seeking(format : Gst::Format, seekable, segment_start, segment_end)
      LibGst.query_set_seeking(to_unsafe.as(LibGst::Query*), format, seekable, Int64.new(segment_start), Int64.new(segment_end))
      nil
    end

    def set_segment(rate, format : Gst::Format, start_value, stop_value)
      LibGst.query_set_segment(to_unsafe.as(LibGst::Query*), Float64.new(rate), format, Int64.new(start_value), Int64.new(stop_value))
      nil
    end

    def uri=(uri)
      LibGst.query_set_uri(to_unsafe.as(LibGst::Query*), uri.to_unsafe)
      nil
    end

    def uri_redirection=(uri)
      LibGst.query_set_uri_redirection(to_unsafe.as(LibGst::Query*), uri.to_unsafe)
      nil
    end

    def uri_redirection_permanent=(permanent)
      LibGst.query_set_uri_redirection_permanent(to_unsafe.as(LibGst::Query*), permanent)
      nil
    end
