module Gdk
  class EventExpose
    ...
    def area
      Gdk::Rectangle.new((to_unsafe.value.area))
    end

    def area=(value : Gdk::Rectangle)
      to_unsafe.value.area = value
    end

    def region
      Cairo::Region.new((to_unsafe.value.region))
    end

    def region=(value : Cairo::Region)
      to_unsafe.value.region = value.to_unsafe.as(LibCairo::Region*)
    end

    def count
      (to_unsafe.value.count)
    end

    def count=(value : Int32)
      to_unsafe.value.count = Int32.new(value)
    end
