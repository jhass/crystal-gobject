module Gdk
  class Keymap < GObject::Object
    @gdk_keymap : LibGdk::Keymap*?
    def initialize(@gdk_keymap : LibGdk::Keymap*)
    end

    def to_unsafe
      @gdk_keymap.not_nil!.as(Void*)
    end
    ...
    def entries_for_keycode(hardware_keycode, keys, keyvals, n_entries)
      __return_value = LibGdk.keymap_get_entries_for_keycode(to_unsafe.as(LibGdk::Keymap*), UInt32.new(hardware_keycode), keys, keyvals, n_entries)
      __return_value
    end

    def entries_for_keyval(keyval, keys, n_keys)
      __return_value = LibGdk.keymap_get_entries_for_keyval(to_unsafe.as(LibGdk::Keymap*), UInt32.new(keyval), keys, n_keys)
      __return_value
    end
