module Gdk
  class Cursor < GObject::Object
    ...
    def self.new_from_pixbuf(display, pixbuf, x, y)
      __return_value = LibGdk.cursor_new_from_pixbuf((display.to_unsafe as LibGdk::Display*), (pixbuf.to_unsafe as LibGdkPixbuf::Pixbuf*), Int32.cast(x), Int32.cast(y))
      Gdk::Cursor.new(__return_value)
    end

    def self.new_from_surface(display, surface, x, y)
      __return_value = LibGdk.cursor_new_from_surface((display.to_unsafe as LibGdk::Display*), (surface.to_unsafe as LibCairo::Surface*), Float64.cast(x), Float64.cast(y))
      Gdk::Cursor.new(__return_value)
    end
    ...
    def surface(x_hot, y_hot)
      __return_value = LibGdk.cursor_get_surface((to_unsafe as LibGdk::Cursor*), Float64.cast(x_hot), Float64.cast(y_hot))
