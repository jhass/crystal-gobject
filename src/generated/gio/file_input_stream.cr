...
module Gio
  class FileInputStream < InputStream
    ...
    def query_info(attributes, cancellable)
      ...
      __return_value = LibGio.file_input_stream_query_info(to_unsafe.as(LibGio::FileInputStream*), attributes.to_unsafe, cancellable ? cancellable.to_unsafe.as(LibGio::Cancellable*) : nil, pointerof(__error))
      GLib::Error.assert __error
      Gio::FileInfo.new(__return_value)
    end

    def query_info_async(attributes, io_priority, cancellable, callback, user_data)
      __return_value = LibGio.file_input_stream_query_info_async(to_unsafe.as(LibGio::FileInputStream*), attributes.to_unsafe, Int32.new(io_priority), cancellable ? cancellable.to_unsafe.as(LibGio::Cancellable*) : nil, callback ? callback : nil, user_data ? user_data : nil)
      __return_value
    end
