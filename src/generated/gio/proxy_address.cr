...
module Gio
  class ProxyAddress < InetSocketAddress
    ...
    def self.new_internal(inetaddr, port, protocol, dest_hostname, dest_port, username, password)
      __return_value = LibGio.proxy_address_new((inetaddr.to_unsafe as LibGio::InetAddress*), UInt16.cast(port), protocol, dest_hostname, UInt16.cast(dest_port), username, password)
