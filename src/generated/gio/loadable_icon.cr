module Gio
  module LoadableIcon
    def load(size, type, cancellable)
      ...
      __return_value = LibGio.loadable_icon_load(to_unsafe.as(LibGio::LoadableIcon*), Int32.new(size), type.to_unsafe, cancellable && cancellable.to_unsafe.as(LibGio::Cancellable*), pointerof(__error))
    ...
    def load_finish(res, type)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGio.loadable_icon_load_finish(to_unsafe.as(LibGio::LoadableIcon*), res.to_unsafe.as(LibGio::AsyncResult*), type.to_unsafe, pointerof(__error))
      GLib::Error.assert __error
      Gio::InputStream.new(__return_value)
    end
