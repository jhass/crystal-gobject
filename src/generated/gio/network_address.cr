module Gio
  class NetworkAddress < GObject::Object
    ...
    def self.new_internal(hostname, port)
      __return_value = LibGio.network_address_new(hostname, UInt16.cast(port))
    ...
    def self.parse_uri(uri, default_port)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGio.network_address_parse_uri(uri, UInt16.cast(default_port), pointerof(__error))
      GLib::Error.assert __error
      __return_value
    end
