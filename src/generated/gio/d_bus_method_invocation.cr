module Gio
  class DBusMethodInvocation < GObject::Object
    @gio_d_bus_method_invocation : LibGio::DBusMethodInvocation*?
    def initialize(@gio_d_bus_method_invocation : LibGio::DBusMethodInvocation*)
    end
    ...
    def return_dbus_error(error_name, error_message)
      LibGio.d_bus_method_invocation_return_dbus_error(to_unsafe.as(LibGio::DBusMethodInvocation*), error_name.to_unsafe, error_message.to_unsafe)
      nil
    end

    def return_error_literal(domain, code, message)
      LibGio.d_bus_method_invocation_return_error_literal(to_unsafe.as(LibGio::DBusMethodInvocation*), UInt32.new(domain), Int32.new(code), message.to_unsafe)
      nil
    end

    def return_gerror(error)
      LibGio.d_bus_method_invocation_return_gerror(to_unsafe.as(LibGio::DBusMethodInvocation*), error)
      nil
    end

    def return_value(parameters)
      LibGio.d_bus_method_invocation_return_value(to_unsafe.as(LibGio::DBusMethodInvocation*), parameters ? parameters.to_unsafe.as(LibGLib::Variant*) : nil)
      nil
    end

    def return_value_with_unix_fd_list(parameters, fd_list)
      LibGio.d_bus_method_invocation_return_value_with_unix_fd_list(to_unsafe.as(LibGio::DBusMethodInvocation*), parameters ? parameters.to_unsafe.as(LibGLib::Variant*) : nil, fd_list ? fd_list.to_unsafe.as(LibGio::UnixFDList*) : nil)
      nil
    end
