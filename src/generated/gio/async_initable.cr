module Gio
  module AsyncInitable
    ...
    def init_async(io_priority, cancellable, callback : LibGio::AsyncReadyCallback?, user_data)
      __return_value = LibGio.async_initable_init_async(to_unsafe.as(LibGio::AsyncInitable*), Int32.new(io_priority), cancellable && cancellable.to_unsafe.as(LibGio::Cancellable*), callback && callback, user_data && user_data)
      __return_value
    end
