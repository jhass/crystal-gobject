module Gio
  class DBusNodeInfo
    ...
    def path
      (raise "Expected string but got null" unless (to_unsafe.value.path); ::String.new((to_unsafe.value.path)))
    end

    def path=(value : String)
      to_unsafe.value.path = value.to_unsafe
    end

    def interfaces
      PointerIterator.new((to_unsafe.value.interfaces)) {|__item| Gio::DBusInterfaceInfo.new(__item) }
    end

    def interfaces=(value : Array(Gio::DBusInterfaceInfo))
      to_unsafe.value.interfaces = value
    end

    def nodes
      PointerIterator.new((to_unsafe.value.nodes)) {|__item| Gio::DBusNodeInfo.new(__item) }
    end

    def nodes=(value : Array(Gio::DBusNodeInfo))
      to_unsafe.value.nodes = value
    end
