...
module Gio
  class InetSocketAddress < SocketAddress
    ...
    def self.new_internal(address, port)
      __return_value = LibGio.inet_socket_address_new((address.to_unsafe as LibGio::InetAddress*), UInt16.cast(port))
      Gio::SocketAddress.new(__return_value)
    end

    def self.new_from_string(address, port)
      __return_value = LibGio.inet_socket_address_new_from_string(address, UInt32.cast(port))
      Gio::SocketAddress.new(__return_value)
    end
