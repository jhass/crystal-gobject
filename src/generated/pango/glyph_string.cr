module Pango
  class GlyphString
    ...
    def extents(font, ink_rect, logical_rect)
      LibPango.glyph_string_extents(to_unsafe.as(LibPango::GlyphString*), font.to_unsafe.as(LibPango::Font*), ink_rect, logical_rect)
      nil
    end

    def extents_range(start, end, font, ink_rect, logical_rect)
      LibPango.glyph_string_extents_range(to_unsafe.as(LibPango::GlyphString*), Int32.new(start), Int32.new(end), font.to_unsafe.as(LibPango::Font*), ink_rect, logical_rect)
      nil
    end

    def free
      LibPango.glyph_string_free(to_unsafe.as(LibPango::GlyphString*))
      nil
    end

    def logical_widths(text, length, embedding_level, logical_widths)
      LibPango.glyph_string_get_logical_widths(to_unsafe.as(LibPango::GlyphString*), text.to_unsafe, Int32.new(length), Int32.new(embedding_level), logical_widths)
      nil
    end
    ...
    def index_to_x(text, length, analysis, index, trailing, x_pos)
      LibPango.glyph_string_index_to_x(to_unsafe.as(LibPango::GlyphString*), text.to_unsafe, Int32.new(length), analysis.to_unsafe.as(LibPango::Analysis*), Int32.new(index), trailing, x_pos)
      nil
    end

    def size=(new_len)
      LibPango.glyph_string_set_size(to_unsafe.as(LibPango::GlyphString*), Int32.new(new_len))
      nil
    end

    def x_to_index(text, length, analysis, x_pos, index, trailing)
      LibPango.glyph_string_x_to_index(to_unsafe.as(LibPango::GlyphString*), text.to_unsafe, Int32.new(length), analysis.to_unsafe.as(LibPango::Analysis*), Int32.new(x_pos), index, trailing)
      nil
    end
