module Pango
  ...
  def self.find_paragraph_boundary(text, length, paragraph_delimiter_index, next_paragraph_start)
    __return_value = LibPango.find_paragraph_boundary(text, Int32.new(length), Int32.new(paragraph_delimiter_index), Int32.new(next_paragraph_start))
    __return_value
  end
  ...
  def self.gravity_get_for_script_and_width(script, wide, base_gravity, hint)
    __return_value = LibPango.gravity_get_for_script_and_width(script, Bool.new(wide), base_gravity, hint)
    __return_value
  end
  ...
  def self.lookup_aliases(fontname, families, n_families)
    __return_value = LibPango.lookup_aliases(fontname, families, Int32.new(n_families))
    __return_value
  end
  ...
  def self.parse_enum(type, str, value, warn, possible_values)
    __return_value = LibPango.parse_enum(UInt64.new(type), str && str, Int32.new(value), Bool.new(warn), possible_values)
    __return_value
  end
  ...
  def self.parse_stretch(str, stretch, warn)
    __return_value = LibPango.parse_stretch(str, stretch, Bool.new(warn))
    __return_value
  end

  def self.parse_style(str, style, warn)
    __return_value = LibPango.parse_style(str, style, Bool.new(warn))
    __return_value
  end

  def self.parse_variant(str, variant, warn)
    __return_value = LibPango.parse_variant(str, variant, Bool.new(warn))
    __return_value
  end

  def self.parse_weight(str, weight, warn)
    __return_value = LibPango.parse_weight(str, weight, Bool.new(warn))
    __return_value
  end
  ...
  def self.scan_int(pos, _out)
    __return_value = LibPango.scan_int(pos, Int32.new(_out))
    __return_value
  end
