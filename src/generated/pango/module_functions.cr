module Pango
  ...
  def self.find_base_dir(text, length)
    __return_value = LibPango.find_base_dir(text, Int32.new(length))
    __return_value
  end

  def self.find_paragraph_boundary(text, length, paragraph_delimiter_index, next_paragraph_start)
    __return_value = LibPango.find_paragraph_boundary(text, Int32.new(length), paragraph_delimiter_index, next_paragraph_start)
    __return_value
  end
  ...
  def self.lookup_aliases(fontname, families, n_families)
    __return_value = LibPango.lookup_aliases(fontname, families, n_families)
    __return_value
  end
  ...
  def self.parse_enum(type, str, value, warn, possible_values)
    __return_value = LibPango.parse_enum(UInt64.new(type), str, value, warn, possible_values)
    __return_value
  end

  def self.parse_markup(markup_text, length, accel_marker, attr_list, text, accel_char)
    __error = Pointer(LibGLib::Error).null
    __return_value = LibPango.parse_markup(markup_text, Int32.new(length), UInt8.new(accel_marker), attr_list, text, accel_char, pointerof(__error))
    GLib::Error.assert __error
    __return_value
  end

  def self.parse_stretch(str, stretch : Pango::Stretch, warn)
    __return_value = LibPango.parse_stretch(str, stretch, warn)
    __return_value
  end

  def self.parse_style(str, style : Pango::Style, warn)
    __return_value = LibPango.parse_style(str, style, warn)
    __return_value
  end

  def self.parse_variant(str, variant : Pango::Variant, warn)
    __return_value = LibPango.parse_variant(str, variant, warn)
    __return_value
  end

  def self.parse_weight(str, weight : Pango::Weight, warn)
    __return_value = LibPango.parse_weight(str, weight, warn)
    __return_value
  end
  ...
  def self.shape_full(item_text, item_length, paragraph_text, paragraph_length, analysis, glyphs)
    __return_value = LibPango.shape_full(item_text, Int32.new(item_length), paragraph_text, Int32.new(paragraph_length), analysis.to_unsafe.as(LibPango::Analysis*), glyphs.to_unsafe.as(LibPango::GlyphString*))
    __return_value
  end
