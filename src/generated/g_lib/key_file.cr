module GLib
  class KeyFile
    ...
    def boolean_list(group_name, key, length)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.key_file_get_boolean_list(to_unsafe.as(LibGLib::KeyFile*), group_name.to_unsafe, key.to_unsafe, length, pointerof(__error))
      GLib::Error.assert __error
    ...
    def comment(group_name, key)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.key_file_get_comment(to_unsafe.as(LibGLib::KeyFile*), group_name ? group_name.to_unsafe : nil, key.to_unsafe, pointerof(__error))
      GLib::Error.assert __error
    ...
    def double_list(group_name, key, length)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.key_file_get_double_list(to_unsafe.as(LibGLib::KeyFile*), group_name.to_unsafe, key.to_unsafe, length, pointerof(__error))
      GLib::Error.assert __error
      PointerIterator.new(__return_value) {|__item| __item }
    end
    ...
    def integer_list(group_name, key, length)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.key_file_get_integer_list(to_unsafe.as(LibGLib::KeyFile*), group_name.to_unsafe, key.to_unsafe, length, pointerof(__error))
      GLib::Error.assert __error
      PointerIterator.new(__return_value) {|__item| __item }
    end
    ...
    def keys(group_name, length)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.key_file_get_keys(to_unsafe.as(LibGLib::KeyFile*), group_name.to_unsafe, length, pointerof(__error))
      GLib::Error.assert __error
      PointerIterator.new(__return_value) {|__item| (raise "Expected string but got null" unless __item; ::String.new(__item)) }
    end
    ...
    def locale_string(group_name, key, locale)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.key_file_get_locale_string(to_unsafe.as(LibGLib::KeyFile*), group_name.to_unsafe, key.to_unsafe, locale ? locale.to_unsafe : nil, pointerof(__error))
      GLib::Error.assert __error
      (raise "Expected string but got null" unless __return_value; ::String.new(__return_value))
    end
    ...
    def locale_string_list(group_name, key, locale, length)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.key_file_get_locale_string_list(to_unsafe.as(LibGLib::KeyFile*), group_name.to_unsafe, key.to_unsafe, locale ? locale.to_unsafe : nil, length, pointerof(__error))
      GLib::Error.assert __error
      PointerIterator.new(__return_value) {|__item| (raise "Expected string but got null" unless __item; ::String.new(__item)) }
    end
    ...
    def string_list(group_name, key, length)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.key_file_get_string_list(to_unsafe.as(LibGLib::KeyFile*), group_name.to_unsafe, key.to_unsafe, length, pointerof(__error))
      GLib::Error.assert __error
      PointerIterator.new(__return_value) {|__item| (raise "Expected string but got null" unless __item; ::String.new(__item)) }
    end
    ...
    def remove_comment(group_name, key)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.key_file_remove_comment(to_unsafe.as(LibGLib::KeyFile*), group_name ? group_name.to_unsafe : nil, key ? key.to_unsafe : nil, pointerof(__error))
      GLib::Error.assert __error
      __return_value
    end
    ...
    def set_comment(group_name, key, comment)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.key_file_set_comment(to_unsafe.as(LibGLib::KeyFile*), group_name ? group_name.to_unsafe : nil, key ? key.to_unsafe : nil, comment.to_unsafe, pointerof(__error))
      GLib::Error.assert __error
      __return_value
    end
