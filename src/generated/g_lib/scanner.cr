module GLib
  class Scanner
    ...
    def destroy
      LibGLib.scanner_destroy(to_unsafe.as(LibGLib::Scanner*))
      nil
    end
    ...
    def input_file(input_fd)
      LibGLib.scanner_input_file(to_unsafe.as(LibGLib::Scanner*), Int32.new(input_fd))
      nil
    end

    def input_text(text, text_len)
      LibGLib.scanner_input_text(to_unsafe.as(LibGLib::Scanner*), text.to_unsafe, UInt32.new(text_len))
      nil
    end

    def lookup_symbol(symbol)
      LibGLib.scanner_lookup_symbol(to_unsafe.as(LibGLib::Scanner*), symbol.to_unsafe)
      nil
    end
    ...
    def scope_add_symbol(scope_id, symbol, value)
      LibGLib.scanner_scope_add_symbol(to_unsafe.as(LibGLib::Scanner*), UInt32.new(scope_id), symbol.to_unsafe, value ? value : nil)
      nil
    end

    def scope_lookup_symbol(scope_id, symbol)
      LibGLib.scanner_scope_lookup_symbol(to_unsafe.as(LibGLib::Scanner*), UInt32.new(scope_id), symbol.to_unsafe)
      nil
    end

    def scope_remove_symbol(scope_id, symbol)
      LibGLib.scanner_scope_remove_symbol(to_unsafe.as(LibGLib::Scanner*), UInt32.new(scope_id), symbol.to_unsafe)
      nil
    end
    ...
    def sync_file_offset
      LibGLib.scanner_sync_file_offset(to_unsafe.as(LibGLib::Scanner*))
      nil
    end

    def unexp_token(expected_token : GLib::TokenType, identifier_spec, symbol_spec, symbol_name, message, is_error)
      LibGLib.scanner_unexp_token(to_unsafe.as(LibGLib::Scanner*), expected_token, identifier_spec.to_unsafe, symbol_spec.to_unsafe, symbol_name.to_unsafe, message.to_unsafe, Int32.new(is_error))
      nil
    end
