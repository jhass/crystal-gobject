module GLib
  class Regex
    ...
    def match(string, match_options, match_info)
      __return_value = LibGLib.regex_match((to_unsafe as LibGLib::Regex*), string, match_options, (match_info.to_unsafe as LibGLib::MatchInfo*))
      __return_value
    end

    def match_all(string, match_options, match_info)
      __return_value = LibGLib.regex_match_all((to_unsafe as LibGLib::Regex*), string, match_options, (match_info.to_unsafe as LibGLib::MatchInfo*))
      __return_value
    end

    def match_all_full(string, string_len, start_position, match_options, match_info)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.regex_match_all_full((to_unsafe as LibGLib::Regex*), string, Int64.cast(string_len), Int32.cast(start_position), match_options, (match_info.to_unsafe as LibGLib::MatchInfo*), pointerof(__error))
      GLib::Error.assert __error
      __return_value
    end
    ...
    def match_full(string, string_len, start_position, match_options, match_info)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.regex_match_full((to_unsafe as LibGLib::Regex*), string, Int64.cast(string_len), Int32.cast(start_position), match_options, (match_info.to_unsafe as LibGLib::MatchInfo*), pointerof(__error))
      GLib::Error.assert __error
      __return_value
    end
    ...
    def replace(string, string_len, start_position, replacement, match_options)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.regex_replace((to_unsafe as LibGLib::Regex*), string, Int64.cast(string_len), Int32.cast(start_position), replacement, match_options, pointerof(__error))
      GLib::Error.assert __error
      raise "Expected string but got null" unless __return_value; String.new(__return_value)
    end
    ...
    def replace_literal(string, string_len, start_position, replacement, match_options)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.regex_replace_literal((to_unsafe as LibGLib::Regex*), string, Int64.cast(string_len), Int32.cast(start_position), replacement, match_options, pointerof(__error))
      GLib::Error.assert __error
      raise "Expected string but got null" unless __return_value; String.new(__return_value)
    end
    ...
    def split_full(string, string_len, start_position, match_options, max_tokens)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.regex_split_full((to_unsafe as LibGLib::Regex*), string, Int64.cast(string_len), Int32.cast(start_position), match_options, Int32.cast(max_tokens), pointerof(__error))
      GLib::Error.assert __error
