module GLib
  class BookmarkFile
    ...
    def to_unsafe
      @g_lib_bookmark_file.not_nil!.as(Void*)
    end

    def add_application(uri, name, exec)
      __return_value = LibGLib.bookmark_file_add_application(to_unsafe.as(LibGLib::BookmarkFile*), uri.to_unsafe, name && name.to_unsafe, exec && exec.to_unsafe)
      __return_value
    end
    ...
    def app_info(uri, name, exec, count, stamp)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.bookmark_file_get_app_info(to_unsafe.as(LibGLib::BookmarkFile*), uri.to_unsafe, name.to_unsafe, exec.to_unsafe, UInt32.new(count), Int64.new(stamp), pointerof(__error))
      GLib::Error.assert __error
      __return_value
    end
    ...
    def applications(uri, length)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.bookmark_file_get_applications(to_unsafe.as(LibGLib::BookmarkFile*), uri.to_unsafe, UInt64.new(length), pointerof(__error))
      GLib::Error.assert __error
    ...
    def groups(uri, length)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.bookmark_file_get_groups(to_unsafe.as(LibGLib::BookmarkFile*), uri.to_unsafe, UInt64.new(length), pointerof(__error))
      GLib::Error.assert __error
      PointerIterator.new(__return_value) {|__item| (raise "Expected string but got null" unless __item; ::String.new(__item)) }
    end
    ...
    def icon(uri, href, mime_type)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.bookmark_file_get_icon(to_unsafe.as(LibGLib::BookmarkFile*), uri.to_unsafe, href.to_unsafe, mime_type.to_unsafe, pointerof(__error))
      GLib::Error.assert __error
      __return_value
    end
    ...
    def title(uri)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.bookmark_file_get_title(to_unsafe.as(LibGLib::BookmarkFile*), uri && uri.to_unsafe, pointerof(__error))
      GLib::Error.assert __error
      (raise "Expected string but got null" unless __return_value; ::String.new(__return_value))
    end
    ...
    def load_from_data_dirs(file, full_path)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.bookmark_file_load_from_data_dirs(to_unsafe.as(LibGLib::BookmarkFile*), file.to_unsafe, full_path && full_path.to_unsafe, pointerof(__error))
      GLib::Error.assert __error
      __return_value
    end
    ...
    def move_item(old_uri, new_uri)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.bookmark_file_move_item(to_unsafe.as(LibGLib::BookmarkFile*), old_uri.to_unsafe, new_uri && new_uri.to_unsafe, pointerof(__error))
      GLib::Error.assert __error
      __return_value
    end
    ...
    def set_description(uri, description)
      __return_value = LibGLib.bookmark_file_set_description(to_unsafe.as(LibGLib::BookmarkFile*), uri && uri.to_unsafe, description.to_unsafe)
      __return_value
    end

    def set_groups(uri, groups, length)
      __return_value = LibGLib.bookmark_file_set_groups(to_unsafe.as(LibGLib::BookmarkFile*), uri.to_unsafe, groups && groups.to_unsafe, UInt64.new(length))
      __return_value
    end

    def set_icon(uri, href, mime_type)
      __return_value = LibGLib.bookmark_file_set_icon(to_unsafe.as(LibGLib::BookmarkFile*), uri.to_unsafe, href && href.to_unsafe, mime_type.to_unsafe)
      __return_value
    end
    ...
    def set_title(uri, title)
      __return_value = LibGLib.bookmark_file_set_title(to_unsafe.as(LibGLib::BookmarkFile*), uri && uri.to_unsafe, title.to_unsafe)
      __return_value
    end
