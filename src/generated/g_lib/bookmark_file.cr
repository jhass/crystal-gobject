module GLib
  class BookmarkFile
    ...
    def app_info(uri, name, exec, count, stamp)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.bookmark_file_get_app_info(to_unsafe.as(LibGLib::BookmarkFile*), uri, name, exec, count, stamp, pointerof(__error))
      GLib::Error.assert __error
      __return_value
    end
    ...
    def applications(uri, length)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.bookmark_file_get_applications(to_unsafe.as(LibGLib::BookmarkFile*), uri, length, pointerof(__error))
      GLib::Error.assert __error
    ...
    def groups(uri, length)
      __error = Pointer(LibGLib::Error).null
      __return_value = LibGLib.bookmark_file_get_groups(to_unsafe.as(LibGLib::BookmarkFile*), uri, length, pointerof(__error))
      GLib::Error.assert __error
      PointerIterator.new(__return_value) {|__item| (raise "Expected string but got null" unless __item; ::String.new(__item)) }
    end
    ...
    def set_groups(uri, groups, length)
      __return_value = LibGLib.bookmark_file_set_groups(to_unsafe.as(LibGLib::BookmarkFile*), uri, groups, UInt64.new(length))
      __return_value
    end
    ...
    def set_is_private(uri, is_private)
      __return_value = LibGLib.bookmark_file_set_is_private(to_unsafe.as(LibGLib::BookmarkFile*), uri, is_private)
      __return_value
    end
